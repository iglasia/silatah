...
<body class="hold-transition skin-blue sidebar-mini">
			...
			<section class="content">
				<div class="row">
					<div class="col-md-6">
						...
						<div class="box box-header">
							<div class="box-header with-border">
								<h3 class="box-title"><i class="fa fa-line-chart"></i>&nbsp;&nbsp;Grafik Jumlah Penerima Beasiswa per Tahun</h3>
					...
					<div class="col-md-6">
						...
						<div class="box box-header">
							<div class="box-header with-border">
								<h3 class="box-title"><i class="fa fa-pie-chart"></i>&nbsp;&nbsp;Grafik Jumlah Penerima Beasiswa per Program Studi</h3>
						...
						<div class="nav-tabs-custom">
							...
							<div class="tab-content">
								<div class="active tab-pane" id="activity">
									<h3 class="box-title">Tabel Beasiswa Mahasiwa</h3>
									<!-- /.box-header -->
									<table id="example1" class="table table-bordered table-striped">
										...
										<tbody>
											...
											foreach ($data as $dt) { ?>
												<tr>
													<td><?php echo ++$no; ?></td>
													<td hidden><?php echo $dt->idBeasiswa; ?></td>
													<td><?php echo $dt->namaMahasiswa; ?></td>
													<td><?php echo $dt->jenisKelamin; ?></td>
													<td><?php echo $dt->namaProdi; ?></td>
													<td><?php echo $dt->jenisBeasiswa; ?></td>
													<td><?php echo $dt->tahun; ?></td>
													<td>
														<button class="btn btn-primary" title="Ubah Data" onclick="edit_book(<?php echo $dt->idBeasiswa; ?>)"><i class="glyphicon glyphicon-pencil"></i></button>
														<button class="btn btn-danger" title="Hapus Data" onclick="delete_book(<?php echo $dt->idBeasiswa; ?>)"><i class="glyphicon glyphicon-remove"></i></button>
													</td>
												</tr>
											<?php } ?>
										</tbody>
								...
								<div class="tab-pane" id="timeline">
									...
									<a href="<?php echo base_url('Kemahasiswaan/printbeasiswa') ?>" target="_blank" class="btn btn-default"><i class="fa fa-print"></i> Cetak</a>
									<h4>Data Beasiswa per Tahun</h4>
									<table class="table table-bordered" style="border-collapse: collapse; width: 100%; margin: 0 auto;">
										<thead>
											<tr>
												<th rowspan="3">No.</th>
												<th rowspan="3">Jenis Beasiswa</th>
										...
										<tbody>
											<?php $no = 0;
											foreach ($data9 as $dta) { ?>
												<tr>
													<td><?php echo ++$no; ?></td>
													<td><?php echo $dta['jenisBeasiswa']; ?></td>
										...
										<tbody>
											...
											foreach ($data7 as $dta) { ?>
												<tr>
													<td><?php echo ++$no; ?></td>
													<td><?php echo $dta['jenisBeasiswa']; ?></td>
													<td><?php echo $dta['ti']; ?></td>
													<td><?php echo $dta['ts']; ?></td>
													<td><?php echo $dta['tm']; ?></td>
													<td><?php echo $dta['te']; ?></td>
													<td><?php echo $dta['ar']; ?></td>
													<td><?php echo $dta['si']; ?></td>
													<td><?php echo $dta['total']; ?></td>
												</tr>
											<?php } ?>
										</tbody>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h3 class="modal-title">Prestasi</h3>
				</div>
				<div class="modal-body form">
					<form action="#" id="form" class="form-horizontal">
						<input type="hidden" value="" name="idBeasiswa" />
						<div class="form-body">
							<div class="form-group">
								<label class="control-label col-md-3">Nama Mahasiswa</label>
								<div class="col-md-9">
									<select name="NPM" id="NPM" class="form-control select2" style="width: 100%;">
										<?php foreach ($data6 as $dt) { ?>
											<option value="<?php echo $dt['NPM'] ?>"><?php echo $dt['namaMahasiswa'] ?></option>
										<?php } ?>
									</select>
								</div>
							</div>
							...
							<div class="form-group">
								<label class="control-label col-md-3">Jenis Beasiswa</label>
								<div class="col-md-9">
									<input name="jenisBeasiswa" id="jenisBeasiswa" placeholder="Jenis Beasiswa" class="form-control" type="text">
								</div>
							</div>
							<div class="form-group">
	...
	<script type="text/javascript">
		...
		function edit_book(id) {
			...
			$.ajax({
				url: "<?php echo site_url('Kemahasiswaan/ajax_edit3/') ?>/" + id,
				type: "GET",
				dataType: "JSON",
				success: function(data) {

					$('[name="idBeasiswa"]').val(data.idBeasiswa);
					$('[name="NPM"]').val(data.NPM);
					$('[name="jenisKelamin"]').val(data.jenisKelamin);
					$('[name="idProdi"]').val(data.idProdi);
					$('[name="jenisBeasiswa"]').val(data.jenisBeasiswa);
					$('[name="tahun"]').val(data.tahun);


					$('#modal_form').modal('show'); // show bootstrap modal when complete loaded
					$('.modal-title').text('Beasiswa Mahasiswa'); // Set title to Bootstrap modal title
		...
		function save() {
			...
			if ($.trim($("#NPM").val()) === "" || $.trim($("#idProdi").val()) === "" || $.trim($("#jenisBeasiswa").val()) === "" || $.trim($("#tahun").val()) === "" || $.trim($("#jenisKelamin").val()) === "") {
				...
				if (save_method == 'add') {
					url = "<?php echo site_url('Kemahasiswaan/tambahBeasiswa') ?>";
				} else {
					url = "<?php echo site_url('Kemahasiswaan/ubahBeasiswa') ?>";
				}
		...
		function delete_book(id) {
			if (confirm('Are you sure delete this data?')) {
				...
				$.ajax({
					url: "<?php echo site_url('Kemahasiswaan/hapusBeasiswa') ?>/" + id,
					type: "POST",
					dataType: "JSON",
					success: function(data) {
	...
	<script>
		$(function() {
			...
			var lineChartData = {
				labels: [<?php foreach ($data8 as $data) {
										echo "'" . $data['tahun'] . "',";
									} ?>],
				datasets: [{
